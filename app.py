        ...
        #    for u in genre_names:
         #       if u==vd:
          #          break
            #if vd=='neutral':
             #   vd=random.choice(genre)
            st.markdown("***Your detected emotion***")
            genre = st.radio(
                "",
...
# Display the image in the first column
        col1.image(lo, use_column_width=True)

# Store the button in a variable
        button_clicked = col2.button('Run Mood Detection')


    
    # st.write('<style>div.row-widget.stButton>button{float:right; margin-bottom: 50px}</style>', unsafe_allow_html=True)
    if button_clicked:
            vd=run_detection()  
            if vd=='neutral':
